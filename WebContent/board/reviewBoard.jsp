...
<button type="button" class="btn btn-outline-primary text-justify" onclick="location.href='reviewBoard_write.jsp'">후기글 작성</button>
      ...
      <th scope="col">조회수</th>
		...
		<%}else{ %> 
		  	...
		  	for(int i=0;i<pageVo.getPageSize() ;i++){
		  		if(num<1) break;
		  		R_BoardVO vo=list.get(curPos++); //0, 5, 10, 15
		  		num--;
		  	%>
				<tr class="align_center">
					...
					<td class="align_left">
						...
						<%}else{ %>	
							<!-- 제목이 긴 경우 일부만 보여주기 -->					
							<a href="review_countUpdate.jsp?no=<%=vo.getNo() %>">
...
<div class="divPage">
	...
	<%if(pageVo.getFirstPage()>1){ %>
		<li class="page-item"><a href="reviewBoard.jsp?currentPage=<%=pageVo.getFirstPage()-1%>&searchCondition=<%=condition%>&searchKeyword=<%=keyword%>"><span class="page-link">이전으로</span></a></li>
	<%}//if %>
	...
	<%
		for(int i=pageVo.getFirstPage();i<=pageVo.getLastPage();i++){
			...
			if(i == currentPage){%>
				...
				<li class="page-item active"><a class="page-link" href="reviewBoard.jsp?currentPage=<%=i%>&searchCondition=<%=condition%>&searchKeyword=<%=keyword%>"><%=i %></a></li>
			
			<%}else{ %>
				<li class="page-item"><a class="page-link" href="reviewBoard.jsp?currentPage=<%=i%>&searchCondition=<%=condition%>&searchKeyword=<%=keyword%>"><%=i %></a></li>
	...
	<%if(pageVo.getLastPage() < pageVo.getTotalPage()){ %>
		<li class="page-item"><a class="page-link" href="reviewBoard.jsp?currentPage=<%=pageVo.getLastPage()+1%>&searchCondition=<%=condition%>&searchKeyword=<%=keyword%>">다음으로</a></li>
	<%}//if %>
	</ul>
	</nav>
	<!--  페이지 번호 끝 -->
...
<div class="divSearch text-center">
   	<form name="frmSearch" method="post" action='reviewBoard.jsp'>
